...
(async () => {
    ...
    const COLORS = [0xff6633, 0xff33cc, 0xcc33ff, 0x33ccff, 0x33ff66, 0xffff66];
    const NUM_BALLS = 120;
    const MAX_RADIUS = 20;
    const MIN_RADIUS = 5;
    ...
    COLORS.forEach((color, index) => {
        for (let i = 0; i < NUM_BALLS / COLORS.length; i++) {
            ...
            ball.radius = mapRange(distanceToPin, 0, app.screen.width, MIN_RADIUS, MAX_RADIUS);
    ...
    app.ticker.add(() => {
        ...
        mouseOrb.clear();
        mouseOrb.lineStyle(4, 0xff0000, 0.8);
        ...
        balls.forEach(ball => {
            if (!ball.inPin) {
                ...
                if (ball.x <= ball.radius || ball.x >= app.screen.width - ball.radius) {
                    ball.vx *= -1;
                }
                if (ball.y <= ball.radius || ball.y >= app.screen.height - ball.radius) {
                    ball.vy *= -1;
                }
                ...
                ball.radius = mapRange(pinDistance, 0, app.screen.width, MIN_RADIUS, MAX_RADIUS);
